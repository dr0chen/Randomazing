...
class Player(Object):
    def __init__(self, speed: float):
        ...
        self.location.player_enter()
    def detect_collisions(self):
        tiles = []
        for level in current_cells:
            if isinstance(level, LargeCell):
                tiles += sum([list(cell.tiles) for cell in level.cells], [])
            else:
                tiles += list(level.tiles)
        for tile in tiles:
            tile.handle_collision(self)
    ...
    def move_to(self, next_cell):
        ...
        self.current_cell.player_leave()
